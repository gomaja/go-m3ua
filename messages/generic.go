...
func (g *Generic) MessageClassName() string {
	// A Generic is any message this package has no concrete type for, but its
	// class byte was still parsed and is often one RFC 4666 defines — RKM, for
	// instance, has message types assigned but no codecs here. Reporting the
	// class it actually carries lets a caller answer per class, which the RFC
	// requires: Section 4.4.1 has an SGP that does not support registration
	// answer with "Unsupported Message Class", not the type-level error.
	switch g.Class {
	case MsgClassManagement:
		return MsgClassNameManagement
	case MsgClassTransfer:
