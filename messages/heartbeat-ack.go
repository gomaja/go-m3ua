...
func (h *HeartbeatAck) MarshalTo(b []byte) error {
	...
	if param := h.HeartbeatData; param != nil {
		if err := param.MarshalTo(h.Header.Payload[offset:]); err != nil {
			return err
		}
		offset += param.MarshalLen()
	}

	// RFC 4666 Section 3.5.6 requires BEAT Ack to include every parameter from
	// the received BEAT without change, including parameters this version does
	// not recognise.
...
func (h *HeartbeatAck) UnmarshalBinary(b []byte) error {
	*h = HeartbeatAck{}
	var err error
	h.Header, err = parseTypedHeader(b, MsgClassASPSM, MsgTypeHeartbeatAck)
	if err != nil {
		return err
	}
