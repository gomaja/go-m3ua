...
| Sustainable capacity above 2,000 messages/s | **Not measured.** At higher offered rates the interval is wider than the instrument's one-message resolution, so a healthy run straddles it and reads `indeterminate`. The gate can still *fail* a run at these rates — the 40,000/s failure run below returned `growing` on a lower bound of 303,944.7 — it simply cannot *pass* one. What is unreachable is a positive verdict, not a verdict. | A narrower sampling bracket — outstanding work counted locally in the sender rather than inferred over an HTTP round trip. Not a threshold change. |
