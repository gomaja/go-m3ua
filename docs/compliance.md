...
| Original upstream #31 | Read API returned transport-sized or ambiguous results | `ReadData`, the typed `DataMessage`, MTP3-User indications, and `TestWriteDataReturnsThePayloadLength` preserve application payload semantics while exposing message metadata explicitly. This is an API-design disposition, not an RFC defect. |
...
| Original upstream PR #56 | Dynamic Protocol Data writes | `WriteData`, `WriteSignal`, and `Endpoint.MTPTransfer` accept per-message Protocol Data; `TestConcurrentFlowsOnOneAssociationKeepTheirRoutingContexts` and SLS-stream tests keep concurrent message scope independent. |
